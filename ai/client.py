...
class AIClient:
    ...
    async def is_topic_relevant(self, query: str, excerpt: str) -> bool:
        ...
        instruction = (
            ...
            f"«{query}»\n\n"
            "این قطعه متن، خام و مستقیم از یک PDF کتاب درسی استخراج شده. ترتیب کلمات داخل هر خط "
            "ممکنه به‌خاطر مشکل فنی استخراج، جابه‌جا یا نامرتب باشه (این طبیعیه، نادیده‌اش بگیر و "
            "فقط دنبال اصطلاحات و کلمات کلیدی مرتبط با موضوع بگرد، نه روانی جمله‌بندی):\n"
            f"«{excerpt[:1200]}»\n\n"
            "آیا این قطعه واقعاً درباره‌ی همون موضوعیه که دانش‌آموز پرسیده (نه فقط یک اشاره‌ی "
            "گذرا و تصادفی در یک زمینه‌ی کاملاً متفاوت مثل تاریخ یا زندگی‌نامه)؟ فقط دقیقاً یکی "
            "از این دو کلمه رو جواب بده: بله یا خیر."
        )
        messages = [{"role": "user", "content": instruction}]

        try:
            response = await self._client.chat.completions.create(
                model=self._fast_model, messages=messages, max_tokens=300, temperature=0
            )
